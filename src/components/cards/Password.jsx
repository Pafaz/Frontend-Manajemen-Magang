...
export default function PengaturanPasswordCard() {
  ...
  const handleSubmit = async () => {
    ...
    try {
      setStatus('loading');
      setErrors({});
      await axios.post(`${import.meta.env.VITE_API_URL}/update-password`, {
  ...
  const renderIcon = (isValid) => isValid
    ? <CheckCircle size={16} className="mr-1 flex-shrink-0 mt-px text-green-600" />
    : <Info size={16} className="mr-1 flex-shrink-0 mt-px text-blue-500" />;
  ...
  return (
    <div className="bg-white rounded-xl shadow-sm p-6 max-w-8xl w-full">
      ...
      <div className="mb-6">
        ...
        {errors.old_password && (
          <p className="text-xs text-red-500 mt-1">{errors.old_password[0]}</p>
        )}
      </div>
